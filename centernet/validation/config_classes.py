...
@dataclass
class ODAPIresidualBlockCFG(Config):
  weights_dict: Dict = field(repr=False, default=None)
...
@dataclass
class ODAPIhourglassCFG(Config):
  weights_dict: Dict = field(repr=False, default=None)
  ...
  def generate_block_weights(self, weights_dict):
    ...
    for i in range(reps):
      res_config = ODAPIresidualBlockCFG(weights_dict=weights_dict[str(i)])
  ...
  def load_weights(self, layer):
    ...
    if len(self.weights_dict['inner_block']) == 1: # still in an outer hourglass
      ...
      inner_hg_layer = layer.submodules[3]
      ...
      inner_layer = layer.submodules[3]
