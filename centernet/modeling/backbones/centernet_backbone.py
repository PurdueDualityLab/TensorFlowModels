    ...
    def build(self, input_shape):
        if self._pre_layers is None:
            self._pre_layers = tf.keras.Sequential([
                ...
                official_nn_blocks.ResidualBlock(filters=256, use_projection=True, strides=2) # shape errors happening
            ])

        # Create hourglass stacks
        self.hgs = tf.keras.Sequential([nn_blocks.HourglassBlock(order=self._order,
