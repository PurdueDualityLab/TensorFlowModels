...
class ViewController: UIViewController {
  ...
  private var currentModel = MobileNetSSD.modelInfo
  private var usingBackCam = true
...
extension ViewController: InferenceViewControllerDelegate {
  ...
  func didChangeThreadCount(to count: Int) {
    ...
    modelDataHandler = ModelDataHandler(
      modelFileInfo: currentModel,//MobileNetSSD.modelInfo,
      labelsFileInfo: MobileNetSSD.labelsInfo,
      threadCount: count,
      isBackCam: usingBackCam
    )
  }
  
  func didChangeModel(to type: Int, with threads: Int) {
    var version = currentModel
    switch type{
    case 0:
      version = MobileNetSSD.modelInfo
    case 1:
      version = MobileNetSSD.bigModelInfo
    default:
      version = MobileNetSSD.modelInfo
      break
    }
    
    if version == currentModel { return }
    currentModel = version
    modelDataHandler = ModelDataHandler(modelFileInfo: currentModel,labelsFileInfo: MobileNetSSD.labelsInfo, threadCount: threads, isBackCam: usingBackCam)
  }
  
  func didSwapCam(to direction: Int, with threads: Int){
    var toBackCam = false
    switch direction{
    case 0:
      toBackCam = true
    ...
    if (toBackCam == usingBackCam) {return}
    
    usingBackCam = toBackCam
    guard cameraFeedManager.swapCamera(useBack: toBackCam) else{
      print("Camera Error")
      cameraFeedManager.stopSession()
      return
    }
    guard cameraFeedManager.resetOutput() else {
...
//    let tapGesture = UITapGestureRecognizer(target: self, action: #selector(ViewController.didPan(panGesture:)))
//    bottomSheetView.addGestureRecognizer(tapGesture)
//  }
//
//  private func addFullPanGesture() {
//    let panGesture = UIPanGestureRecognizer(target: self, action: #selector(ViewController.didPan(panGesture:)))
