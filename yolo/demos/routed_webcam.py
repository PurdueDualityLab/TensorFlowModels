...
class FastVideo(object):
    ...
    def __init__(self,
        ...
        print(self._width)
        self._fake_cam = pyfakewebcam.FakeWebcam('/dev/video20', 960, 720)#self._width, self._height)
            ...
            while (self._cap.isOpened() and self._running):
                ...
                for i in range(image.shape[0]):
                    ...
                    with tf.device("/CPU:0"):
                        frame = tf.image.resize_with_crop_or_pad(image[i], target_width=960, target_height=720).numpy()
