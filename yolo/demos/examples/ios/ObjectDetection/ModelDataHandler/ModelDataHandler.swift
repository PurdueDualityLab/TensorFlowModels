...
enum MobileNetSSD {
  ...
  static let bigModelInfo: FileInfo = (name: "detect-large", extension: "tflite")
...
class ModelDataHandler: NSObject {
  ...
  private var usingBackCam: Bool

  // MARK: - Initialization

  /// A failable initializer for `ModelDataHandler`. A new instance is created if the model and
  /// labels files are successfully loaded from the app's main bundle. Default `threadCount` is 1.
  // predictions are flipped when using the front cam, need to flip the boxes. (xmax - 1),(xmin - 1)
  init?(modelFileInfo: FileInfo, labelsFileInfo: FileInfo, threadCount: Int = 1, isBackCam: Bool = true) {
    let modelFilename = modelFileInfo.name
    usingBackCam = isBackCam
  ...
  func formatResults(boundingBox: [Float], outputClasses: [Float], outputScores: [Float], outputCount: Int, width: CGFloat, height: CGFloat) -> [Inference]{
    ...
    for i in 0...outputCount - 1 {
      ...
      if usingBackCam{
        ...
        let width = CGFloat(boundingBox[4*i+3]) - CGFloat(boundingBox[4*i+1])
        let height = CGFloat(boundingBox[4*i+2]) - CGFloat(boundingBox[4*i])
        rect.origin.y = CGFloat(boundingBox[4*i])
        rect.origin.x = 1 - CGFloat(boundingBox[4*i+3])//CGFloat(boundingBox[4*i+1])
