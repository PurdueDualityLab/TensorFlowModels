...
class CameraFeedManager: NSObject {
  ...
  private var backCamInUse = false
  
  // MARK: CameraFeedManagerDelegate
  weak var delegate: CameraFeedManagerDelegate?

  // MARK: Initializer
  init(previewView: PreviewView, back: Bool = true) {
    ...
    backCamInUse = back
    self.attemptToConfigureSession(back: back)
  }
   ...
   */
  private func attemptToConfigureSession(back: Bool = false) {
    ...
    self.sessionQueue.async {
      self.configureSession(back: back)
    }
  }
   ...
   */
  private func configureSession(back: Bool = false) {
    ...
    guard addVideoDeviceInput(back: back) == true else {
   ...
   */
  private func addVideoDeviceInput(back: Bool = false) -> Bool {
    ...
    if back{
      device = AVCaptureDevice.Position.back
    }else{
      device = AVCaptureDevice.Position.front
    }
    
    
    guard let camera  = AVCaptureDevice.default(.builtInWideAngleCamera, for: .video, position: device) else {
  ...
  public func swapCamera(useBack: Bool) -> Bool{
    
    if (backCamInUse == useBack){return true}
    backCamInUse = useBack
    
    //remove all inputs
    for i: AVCaptureInput in session.inputs{
      session.removeInput(i)
    }
    
    //get the cam direction
    var device: AVCaptureDevice.Position
    if useBack{
      device = AVCaptureDevice.Position.back
    }else{
    ...
    do{
      let cap = try AVCaptureDeviceInput(device:camera)
      if session.canAddInput(cap){
      ...
      fatalError("could not swapCameras")
    }
  }
  
  func resetOutput() -> Bool{
    for i: AVCaptureOutput in session.outputs{
      session.removeOutput(i)
    }
    return addVideoDataOutput()
  }
