...
class UDPPacket(object):
    IMP_SIZE = 2**16 - 2**8
    def __init__(self, frame_id:int , sqn_number:int, num_packs:int, total_size:int, data:bytes = b""):
    ...
    @property
    def bytes(self):
        return self._frame_id.to_bytes(4, "little") + self._sqn_number.to_bytes(4, "little") + self._num_packs.to_bytes(4, "little") + self._total_size.to_bytes(4, "little") + self._data
    ...
    def split(id, byte_str):
        ...
        while (i < num_packets):
            ...
            pack_list.append(UDPPacket(id, i, num_packets, len(byte_str), byte_str[start_pos:end_pos]))
            start_pos = end_pos
            i += 1
        return pack_list
