...
class GradientAggregator(tf.Module):
  ...
  def __init__(self, train_vars, batch_size, subdivisions, loss_aggregation = "MEAN",  name = "gradientAgg"):
    super().__init__(name=name)
    self._grad = [self.add_grad(name = f"{var.name}$grad",  variable = var) for var in train_vars]
    self._zeros = [self.add_grad(name = f"{var.name}$zeros",  variable = var) for var in train_vars]
  ...
  def update_state(self, values, sample_weight=None):
    ...
    if self._loss_aggregation == "SUM":
      for i in range(len(self._grad)):
        self._grad[i].assign(self._grad[i] * mod + (values[i] / tf.cast(self._subdivisions * self._batch_size, values[i].dtype)))
    elif self._loss_aggregation == "MEAN":
      for i in range(len(self._grad)):
        self._grad[i].assign(self._grad[i] * mod + (values[i] / tf.cast(self._subdivisions, values[i].dtype)))
    else:
      for i in range(len(self._grad)):
...
if __name__ == "__main__":
  ...
  task, model, params = run.load_model(config_path=["/media/vbanna/DATA_SHARE/Research/TensorFlowModelGardeners/yolo/configs/experiments/yolov4-tiny-eval.yaml"], model_dir="")
  ...
  subdiv_ag = GradientAggregator(model.trainable_variables, task._task_config.validation_data.global_batch_size, subdivisions, loss_aggregation="MEAN")
