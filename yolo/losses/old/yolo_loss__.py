...
def gradient_trap(y):
  ...
  def trap(dy):
    def axis(a):
      return tf.range(1, tf.shape(tf.shape(a))[0])
    dya = dy
    

    height = tf.shape(dy)[1]
    width = tf.shape(dy)[2]
    batch = tf.shape(dy)[0]
    num = 3
    dy = tf.reshape(dy, [batch, height, width, num, -1])

    box, conf, classes = tf.split(dy, [4, 1, -1], axis = -1)

    conf = tf.reduce_sum(tf.square(conf), axis = axis(conf))
    classes = tf.reduce_sum(tf.square(classes), axis = axis(classes))
    tf.print(tf.reduce_mean(classes), tf.reduce_mean(conf))
    return dya
...
@tf.custom_gradient
def no_grad_reduce_sum(value, axis):
  value = tf.reduce_sum(value, axis = axis)
  def delta(dy):
    return dy, tf.zeros_like(axis)
  return value, delta

@tf.custom_gradient
def no_grad_reduce_mean(value, axis):
  values = tf.reduce_mean(value, axis = axis)
  def delta(dy):
    count = tf.reduce_sum(tf.ones_like(value), axis = axis)
    tf.print(dy, values, count, summarize = -1)
    return dy/count, tf.zeros_like(axis)
  return values, delta
...
class Yolo_Loss(object):
  ...
  def call_darknet(self, true_counts, inds, y_true, boxes, classes, y_pred):
    ...
    loss = no_grad_reduce_mean(loss, 0)

    # 20. reduce the mean of the losses to use as metrics
    box_loss = tf.stop_gradient(tf.reduce_mean(box_loss))
    conf_loss = tf.stop_gradient(tf.reduce_mean(conf_loss))
    class_loss = tf.stop_gradient(tf.reduce_mean(class_loss))
