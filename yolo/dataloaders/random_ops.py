...
def _jitter_rand(box_jitter = 0.005):
    ...
    """
    global RANDOM_SEED
    jitter_cx = RANDOM_SEED.uniform(minval = -box_jitter, maxval = box_jitter, shape = (), dtype = tf.float32) 
    jitter_cy = RANDOM_SEED.uniform(minval = -box_jitter, maxval = box_jitter, shape = (), dtype = tf.float32)
    jitter_bw = RANDOM_SEED.uniform(minval = -box_jitter, maxval = box_jitter, shape = (), dtype = tf.float32) + 1.0
    jitter_bh = RANDOM_SEED.uniform(minval = -box_jitter, maxval = box_jitter, shape = (), dtype = tf.float32) + 1.0
    return jitter_cx, jitter_cy, jitter_bw, jitter_bh 

def _translate_rand(image_jitter = 0.1):
    global RANDOM_SEED
    translate_x = RANDOM_SEED.uniform(minval = -image_jitter, maxval = image_jitter, shape = (), dtype = tf.float32)
    translate_y = RANDOM_SEED.uniform(minval = -image_jitter, maxval = image_jitter, shape = (), dtype = tf.float32)
    return translate_x, translate_y

def _box_scale_rand(min_val = 10, max_val = 19, randscale = 13, frac_dat_scale = 0.5):
    global RANDOM_SEED
    scale_q = RANDOM_SEED.uniform(minval = 0, maxval = tf.cast(1/frac_dat_scale, dtype = tf.int32), shape = (), dtype = tf.int32)
    if scale_q == 0:
        randscale = RANDOM_SEED.uniform(minval = 10, maxval = 19, shape = (), dtype = tf.int32)
    return randscale
