...
def preprocessing(dataset,
                  ...
                  batch_size,
                  size,
                  data_augmentation_split=100,
                  preprocessing_type="detection",
                  shuffle_flag=False,
                  anchors=None,
                  masks=None,
                  fixed=False,
    ...
    Dataset_Parser = Classification_Parser([224, 224],
                                           50)  # ADD CONFIG FILE SUPPORT
    _normalize, _data_augmentation = Dataset_Parser.parse_fn(
        is_training=False), Dataset_Parser.parse_fn(is_training=True)
    ...
    data_augmentation_dataset = data_augmentation_dataset.map(
        _data_augmentation, num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
Train, Info = tfds.load('imagenet_a',
                        decoders={
