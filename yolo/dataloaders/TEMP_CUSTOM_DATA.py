...
def xml_json_converter(label_folder_path):
    ...
    for x in glob.glob(label_folder_path):
        with open(x) as xml_file:
            ...
            xml_file.close()
        with open(json_dir+os.path.basename(os.path.normpath(x))[:-4]+".json", "w") as json_file:
...
def json_sereloizer(json_string):
    dictionary = json.loads(json.loads(json.dumps(json_string.numpy().decode('utf-8'))))
    obj= dictionary["annotation"]["object"]
    area = int(dictionary["annotation"]["size"]["width"]) * int(dictionary["annotation"]["size"]["height"])
    return obj["name"], 1,int(obj["bndbox"]["xmin"]), int(obj["bndbox"]["ymin"]), int(obj["bndbox"]["xmax"]), int(obj["bndbox"]["ymax"]), area

def parser(fp):
    label = tf.strings.split(fp, '/')[-1]
    f = tf.io.read_file(fp)
    name, identifier, xmin, ymin, xmax, ymax, area = tf.py_function(json_sereloizer, [f], [tf.string, tf.int64, tf.float32, tf.float32, tf.float32, tf.float32, tf.int64])
    return {'id' : identifier, 'name': name, 'bbox' : tf.concat([xmin,ymin,xmax,ymax], 0), 'area': area}
...
class customdataloader:
    ...
    def construct_dataset(self):
        ...
        fp = list(zip(glob.glob(self.image_folder_path), glob.glob(self.label_folder_path)))
