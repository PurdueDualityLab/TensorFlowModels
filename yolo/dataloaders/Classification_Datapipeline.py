...
class Classification_Parser(Parser):
    ...
    def __init__(self,
        ...
        self._scale = [tf.cast(scale[0], tf.float32)[None], tf.cast(scale[1], tf.float32)[None]]
        if dtype == 'float32':
    ...
    def _parse_train_data(self, decoded_tensors):
        ...
        if self._aug_rand_zoom:
            scale = tf.py_function(_rand_number, [self._scale[0], self._scale[1]], [tf.float32])
        ...
        image = tf.image.resize_with_pad(image, target_width=self._output_size[0], target_height=self._output_size[1])
        ...
        if "objects" in decoded_tensors:
            ...
            return image, tf.one_hot(decoded_tensors['label'], self._num_classes)
    ...
    def _parse_eval_data(self, decoded_tensors):
        ...
        """
        image = tf.cast(decoded_tensors['image'], tf.float32)
        image = tf.image.resize_with_pad(image, target_width=self._output_size[0], target_height=self._output_size[1]) # Final Output Shape
