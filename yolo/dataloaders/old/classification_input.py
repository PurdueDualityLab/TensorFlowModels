...
class Parser(parser.Parser):
  ...
  def _parse_train_data(self, decoded_tensors):
    ...
    #   delta = 0.1 #preprocessing_ops.random_uniform_strong(-0.1, 0.1)
    #   i_h = i_h + delta  # Hue
    #   i_h = tf.clip_by_value(i_h, 0.0, 1.0)
    # if self._aug_rand_saturation:
    #   delta = 0.75 #preprocessing_ops.random_scale(0.75)
    #   i_s = i_s * delta
    ...
    if self._aug_rand_hue:
      delta = preprocessing_ops.random_uniform_strong(-0.1, 0.1)
      image = tf.image.adjust_hue(image, delta)
    if self._aug_rand_saturation:
      delta = preprocessing_ops.random_scale(0.75)
    ...
    if self._aug_rand_aspect:
      aspect = preprocessing_ops.random_scale(0.75)
