...
class Trainer(tf.keras.Model):
    ...
    def test_step(self, inputs):
        return self._task.validation_step(inputs, self._model, metrics=self._metrics)
    ...
    def build_datasets(self, take = 1000):
        train_data = self.build_inputs(self._task.task_config.train_data, is_training=True).take(1000)
        validation_data = self.build_inputs(self._task.task_config.validation_data, is_training=False).take(1000)
        return train_data, validation_data

    def train(self, train_data, test_data, optimizer, epochs = 1, callbacks = None):
        self.compile(optimizer)
        self.fit(train_data, validation_data=test_data, epochs= epochs, callbacks=callbacks, verbose = 0)
        return
...
if __name__ == "__main__":
    ...
    schedule = schedule.Schedule(1000, 0.001, total_steps = size * EPOCHS, type = "STEPWISE", step_changes = step_changes, decay_factor = 0.1)
    ...
    try:
        trainer.train(train, test, optimizer = optimizer, epochs= EPOCHS, callbacks= callbacks)
