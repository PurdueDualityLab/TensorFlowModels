...
class Schedule(object):
    def __init__(self, warmup_steps, learning_rate, total_steps, type='COSINE', hold_base_rate_steps = 0, offset = 0.0, **kwargs):
        self._global_step = 0
    ...
    def cosine_lr(self):
        ...
        learning_rate = 0.5 * self._learning_rate * (
            1 + np.cos(np.pi * (self._global_step - self._warmup_steps - self._hold_lr_befor_decay) / float(self._total_steps - self._warmup_steps - self._hold_lr_befor_decay)))
        return learning_rate
    
    def stepwise_lr(self, step_changes, decay_factor):
        curr_lr = self._curr_lr
        if (self._global_step - self._warmup_steps) in step_changes: # Does subtracting the self._warmup_steps belong here?
