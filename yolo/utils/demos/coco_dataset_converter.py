...
def serialize(img_bytes, source_id, height, width, xmins, xmaxs, ymins, ymaxs, labels, is_crowds, areas):
...
def convert(img_folder, annotations_file, record):
  ...
  with tf.io.TFRecordWriter(record) as writer:
    for img in tqdm(dic['images']):
      ...
      tfexample = serialize(img_bytes, source_id, height, width, xmins, xmaxs, ymins, ymaxs, labels, is_crowds, areas)
      writer.write(tfexample)

if __name__ == '__main__':
  convert('coco_subset/train2017', 'coco_subset/train2017_labels.json', 'train.tfrecord')
  convert('coco_subset/val2017', 'coco_subset/val2017_labels.json', 'val.tfrecord')
