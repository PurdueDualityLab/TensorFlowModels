...
class TBiFPN(tf.keras.Model):
  ...
  def merge_levels(self, source, query, oquery = None):
    ...
    attention_sp = WindowedIdentityAttention(
      query_window_size, 
      num_heads = num_heads, 
      qkv_bias=True, 
      qk_scale=None, 
    ...
    nq = self.layer_norm(query)
    ns = self.layer_norm(source)
    attn1 = attention_sp([nq, ns])
    x = tf.keras.layers.Add()([source, attn1])
    # attn2 = attention_dw([nq, ns])
    # x = tf.keras.layers.Add()([source, attn1, attn2])
