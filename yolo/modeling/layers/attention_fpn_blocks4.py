...
class IdentityAttention(tf.keras.layers.Layer):
  ...
  use spatial to improve identity 
  """
  ...
  def call(self, qkv, mask = None, training = None):
    ...
    #x = tf.einsum("bhij,bhjk->bihk", attn, v)
    x = tf.matmul(attn, v)
    x = tf.transpose(x, perm = (0, 3, 1, 2)) # move heads to be merged with features
    x = tf.reshape(x, [-1, qWh, qWw, C])
    return x, attn
...
class SpatialIdentitySelfAttention(tf.keras.layers.Layer):
  ...
  def __init__(self, 
    ...
    self._filters_a = filters_a
    self._filters_b = filters_b
  ...
  def build(self, input_shape):
    ...
    filters_b = self._filters_b or input_shape[-1]
    ...
    head_dims = filters_a//self._num_heads
    self.scale = self._qk_scale or head_dims ** -0.5 # x/sqrt(num_heads) = x/sqrt(d_k)

    qWh, qWw = self._window_size
    kWh, kWw = self._window_size
    self._window_size_q = [qWh, qWw] 
    self._window_size_k = [kWh, kWw] 
  ...
  def reshape_spatial_projection(self, x, N, C):
    x = tf.reshape(x, [-1, N, self._num_heads, C // self._num_heads])
    x = tf.transpose(x, perm=(0, 2, 1, 3))
    return x
  ...
  def call_spatial(self, q, k, v, training = None):
    ...
    _, kWh, kWw, C = k.shape
    kN = kWh * kWw 
    q = self.reshape_spatial_projection(q, qN, C)
    k = self.reshape_spatial_projection(k, kN, C)
    v = self.reshape_spatial_projection(v, kN, C)
  ...
  def call_identity(self, q, k, v, training = None):
    _, qWh, qWw, C = q.shape
    qN = qWh * qWw 
    _, kWh, kWw, C = k.shape
    kN = kWh * kWw 
    q = self.reshape_identity_projection(q, qN, C)
    k = self.reshape_identity_projection(k, kN, C)
    v = self.reshape_identity_projection(v, kN, C)
...
class WindowedIdentityAttention(tf.keras.layers.Layer):
  def __init__(self, 
               window_size, 
               num_heads, # identity groups
               kernel_size = 1,
  ...
  def build(self, input_shape):
    ...
    self.q = tf.keras.layers.DepthwiseConv2D(
                              3, strides = 1, 
                              padding = "same", 
                              use_bias = False, 
                              kernel_initializer='TruncatedNormal',
    ...
    self.k = tf.keras.layers.DepthwiseConv2D(
                              3, strides = 1, 
                              padding = "same", 
                              use_bias = False, 
                              kernel_initializer='TruncatedNormal',
    ...
    self.v = tf.keras.layers.DepthwiseConv2D(
                              3, strides = 1, 
                              padding = "same", 
                              use_bias = False, 
                              kernel_initializer='TruncatedNormal',
  ...
  def call(self, inputs, mask = None, training = None):
    querys, source = inputs 

    # print(querys.shape)
    querys = self.q(querys)
    keys = self.k(source)
    values = self.v(source)

    (_, querys, x_shifts, 
    x_split_size, H, W, C, Hp, Wp) = pad_and_shift_input(querys, 
                                        self._window_size, 
                                        (0, 0), 
                                        shift = False)
    ...
    (_, keys, _, 
    _, _, _, 
    _, _, _) = pad_and_shift_input(keys, 
                                        self._window_size, 
                                        (0, 0), 
                                        shift = False)
    ...
    (_, values, _, 
    _, _, _, 
    _, _, _) = pad_and_shift_input(values, 
                                        self._window_size, 
                                        (0, 0), 
                                        shift = False)
    # querys = self.q(querys)
    ...
    x = upad_and_unshift(x, x_split_size, Hp,Wp, self._window_size, x_shifts, H, W)
    return x
...
class TBiFPN(tf.keras.Model):
  ...
  def merge_levels(self, source, query, oquery = None, fpn_only = False):
    ...
    attention = SpatialIdentitySelfAttention(
      3, 
      query_window_size,
      filters_b=exp1_features,
      use_sync_bn=True, 
      use_bn=True,
      num_heads = num_heads, 
      qkv_bias=True, 
