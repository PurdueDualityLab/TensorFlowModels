...
from yolo.modeling.building_blocks import DarkResidual
...
class DarkResidualTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_pass_through(self,  width, height, filters, downsample):
        ...
        test_layer = DarkResidual(filters = filters, downsample=downsample)
        outx = test_layer(x)
        print(outx)
        print(outx.shape.as_list())
        self.assertAllEqual(outx.shape.as_list(), [None, np.ceil(width/mod), np.ceil(height/mod), filters])
        return
    ...
    def test_gradient_pass_though(self, filters, width, height, downsample):
        ...
        test_layer = DarkResidual(filters, downsample = downsample)
