...
class DarkUpsampleRouteTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_pass_through(self, width, height, filters, upsampling_size):
        ...
        x_route = ks.Input(shape=(width * upsampling_size[0], height * upsampling_size[1], filters))
        test_layer = DarkUpsampleRoute(filters=filters, upsampling_size=upsampling_size)
        outx = test_layer([x_conv, x_route])
        self.assertAllEqual(outx.shape.as_list(), [None, width * upsampling_size[0], height * upsampling_size[1], filters * 2])
    ...
    def test_gradient_pass_though(self, width, height, filters, upsampling_size):
        ...
        x_route = tf.Variable(initial_value=init(
            shape=(1, width * upsampling_size[0], height * upsampling_size[1], filters), dtype=tf.float32))
