...
class BackBoneTest(tf.test.TestCase, parameterized.TestCase):
    ...
    def test_pass_through(self, model, input_shape):
        if model == "darknet53":
            check = {
                1024: [input_shape[0], input_shape[1]//32, input_shape[2]//32, 255], 
                512: [input_shape[0], input_shape[1]//16, input_shape[2]//16, 255], 
                256: [input_shape[0], input_shape[1]//8, input_shape[2]//8, 255], 
                }
            name = "spp"
        elif model == "darknet_tiny":
            check = {
                1024: [input_shape[0], input_shape[1]//32, input_shape[2]//32, 340], 
                256: [input_shape[0], input_shape[1]//16, input_shape[2]//16, 340], 
                }
            name = "tiny"
        else:
            check = None
    ...
    def test_gradient_pass_though(self, model, input_shape):
        ...
        init = tf.random_normal_initializer()
        x = tf.Variable(initial_value = init(shape = input_shape, dtype = tf.float32))
        y = {key: tf.Variable(initial_value = init(shape = value, dtype = tf.float32)) for key, value in check.items()}
