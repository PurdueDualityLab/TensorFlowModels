...
class DarkNet53(ks.Model):
    ...
    def _load_backbone_weights(self, config, weights):
        ...
        for i, (layer, weights) in enumerate(zip(self.back_bone.layers, weight_list)):
...
model = DarkNet53(classes = 2, load_backbone_weights = True, weights_file = "yolov3_416.weights")
model.get_summary()
y = model(x)
