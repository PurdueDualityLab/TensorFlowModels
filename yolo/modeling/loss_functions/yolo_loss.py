...
class Yolo_Loss(ks.losses.Loss):
    def __init__(
        ...
        self._random = 0 if random is None else random
...
x = tf.Variable(initial_value=ones(
    (2, 13, 13, n * (classes + 5)), dtype=tf.float32))
y = tf.Variable(initial_value=ones(
    (2, 13, 13, n * (classes + 5)), dtype=tf.float32))
...
# option_find_int(linked_list, key, defaults_value) -> same as above but
# print nothing
