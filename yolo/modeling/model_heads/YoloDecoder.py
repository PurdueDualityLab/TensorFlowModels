...
class FPNTail(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self._route_conv = DarkConv(filters=self._filters // 2,
                               kernel_size=(1, 1),
                               strides=(1, 1),
                               padding="same",
        ...
        if self._upsample:
            self._process_conv = DarkConv(filters=self._filters // 4,
                            kernel_size=(1, 1),
                            strides=(1, 1),
                            padding="same",
                            activation=self._activation,
                            ...
                            norm_epsilon= self._norm_epsilon, 
                            norm_momentum= self._norm_momentum)
            self._upsampling_block = ks.layers.UpSampling2D(size=self._upsample_size)   
        return 
    ...
    def build(self, inputs):
        ...
        for level, depth in zip(reversed(range(self._min_level, self._max_level + 1)), self._depths):
            ...
            if level != self._max_level: 
                self.resamples[str(level)] = DarkRoute(filters=depth//2, 
                                                    activation=self._activation,
                                                    use_sync_bn= self._use_sync_bn,
                                                    kernel_regularizer=self._kernel_regularizer,
                                                    kernel_initializer=self._kernel_initializer,
                                                    ...
                                                    norm_epsilon= self._norm_epsilon, 
                                                    norm_momentum= self._norm_momentum)
                self.preprocessors[str(level)] = DarkRouteProcess(filters = depth, 
...
class YoloRoutedDecoder(ks.Model):
    ...
    def build(self, inputs):
        ...
        for level, depth in zip(reversed(range(self._min_level, self._max_level + 1)), self._depths):
            ...
            if level == self._max_level:
                self.preprocessors[str(level)] = DarkRouteProcess(filters = depth,
                                                            repetitions = self._max_level_process_len + 2 * (1 if self._embed_spp else 0),
                ...
                self.resamples[str(level)] = DarkUpsampleRoute(filters= depth // 2,
                                                        activation=self._activation,
                                                        use_sync_bn= self._use_sync_bn,
                                                        kernel_regularizer=self._kernel_regularizer,
                                                        kernel_initializer=self._kernel_initializer,
                ...
                self.preprocessors[str(level)] = DarkRouteProcess(filters = depth,
                                                            ...
                                                            norm_epsilon= self._norm_epsilon, 
                                                            norm_momentum= self._norm_momentum)
            self.outputs[str(level)] = DarkConv(filters= self._output_conv,
                                        kernel_size = (1, 1),
                                        strides = (1, 1),
                                        padding = "same",
                                        use_bn = False,
...
class YoloFPNDecoder(ks.Model):
    ...
    def build(self, inputs):
        ...
        for level, depth in zip(range(self._min_level, self._max_level + 1), self._depths):
            print(level, depth)
            if level == self._min_level:
                self.preprocessors[str(level)] = DarkRouteProcess(filters = depth * 2,
                                                            repetitions = self._max_level_process_len + 2 * (1 if self._embed_spp else 0),
                                                            insert_spp = self._embed_spp, 
                                                            activation=self._activation,
                                                            use_sync_bn= self._use_sync_bn,
                ...
                self.resamples[str(level)] = DarkRoute(filters= depth,
                                                    downsample= True,
                                                    activation=self._activation,
                                                    use_sync_bn=self._use_sync_bn,
                                                    kernel_regularizer=self._kernel_regularizer,
...
class YoloDecoder(ks.Model):
    ...
    def build(self, inputs):
        ...
        if self._embed_fpn:
            self._fpn = YoloFPN(fpn_path_len=self._fpn_path_len, 
                                ...
                                kernel_regularizer=self._kernel_regularizer, 
                                bias_regularizer=self._bias_regularizer)
            self._decoder = YoloFPNDecoder(classes= self._classes, 
            ...
            self._fpn = None
            self._decoder = YoloRoutedDecoder(classes= self._classes, 
                                        boxes_per_level=self._boxes_per_level, 
                                        output_extras=self._output_extras, 
                                        path_process_len=self._path_process_len, 
                                        max_level_process_len=self._max_level_process_len, 
    ...
    def get_loss_attributes(self, xy_exponential = False, exp_base = 2, xy_scale_base = "default_value"):
        start = 0
        boxes = {}
        path_scales = {}
        scale_x_y = {}
        ...
        if xy_scale_base == "default_base":
            xy_scale_base = 0.05
            xy_scale_base = xy_scale_base / (self._boxes_per_level * (self._max_level - self._min_level + 1) - 1)
...
if __name__ == "__main__":
    ...
    backbone = Darknet(model_id="darknettiny", min_level=2, max_level=5, input_specs=(416, 416, 3))
    decoder = YoloDecoder(classes = 80, boxes_per_level = 3, embed_spp= False, embed_fpn = False, max_level_process_len=None, path_process_len=6)
    ...
    print(decoder.get_loss_attributes(xy_exponential = True, xy_scale_base="default_value"))
