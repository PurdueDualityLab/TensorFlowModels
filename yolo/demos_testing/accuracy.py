...
def accuracy(model = "regular", metrics = None):
    ...
    dataset, Info = tfds.load('coco', split='validation', with_info=True, shuffle_files=True, download=False)
    ...
    for image, label in dataset:
        ...
        counter += 1
        print("recall: %0.3f, accuracy: %0.3f, bbox iou: %0.3f, number of samples: %0.3f"%(total_recall/counter, total_accuracy/counter, total_iou/counter, counter), end = "\r")
    return
...
def test_iou_match(gt, pred):
    ...
    for i in range(batch_size):
        ...
        try:
            mean_iou = np.mean(np.trim_zeros(np.max(temp[i], axis = -1), trim = 'b'))
            if not np.isnan(mean_iou):
                best_iou.append(np.mean(np.trim_zeros(np.max(temp[i], axis = -1), trim = 'b')))
            else:
                best_iou.append(0)    
        except:
            best_iou.append(0)
