...
# def _update_tensor_arrays(y, x, anchor_idx, boxes, classes, update_index,
#                           update, i, iou = 1.0):
  
#   const = tf.cast(tf.convert_to_tensor([1.]), dtype=boxes.dtype)
#   iou = tf.cast(tf.convert_to_tensor([iou]),  dtype=boxes.dtype)
#   update_index = update_index.write(i, [y, x, anchor_idx])
#   update = update.write(i, K.concatenate([boxes, const, classes, iou]))
#   i += 1
#   return update_index, update, i


# def _use_tie_breaker(anchors, ious, mask, i, box_id, depth_track, x, y, boxes,
#                      classes, update_index, update):
#   mask = tf.cast(mask, dtype=boxes.dtype)
#   rand_update = 0.0
#   for anchor_id in range(tf.shape(anchors)[-1]):
#     anchor = anchors[box_id, anchor_id]
#     iou = ious[box_id, anchor_id]
#     # if anchor < 0:
#     #   break
...
#           K.argmax(tf.cast(index, dtype=tf.int32)), dtype=tf.int32)
#       # determine if the index was used or not
#       used = depth_track[y[box_id], x[box_id], anchor_idx]
#       # defualt used upadte value
#       uid = 1
...
#         # create random number to trigger a replacment if the cell
#         # is used already
#         update_index, update, i = _update_tensor_arrays(y[box_id],
#                                                         x[box_id], anchor_idx,
#                                                         boxes[box_id],
#                                                         classes[box_id],
#                                                         update_index, update, i, 
...
#         if rand_update > 0.3:
#           uid = 2
#           update_index, update, i = _update_tensor_arrays(y[box_id], x[box_id], anchor_idx,
#               boxes[box_id], classes[box_id], update_index, 
#                                             update, i, iou = iou)
#         else:
#           uid = 0
...
#       depth_track = tf.tensor_scatter_nd_update(
#           depth_track,
#           [(y[box_id], x[box_id], anchor_idx)], [uid])

#   return update_index, update, depth_track, i

# def _build_grided_gt(y_true, mask, size, num_classes, dtype, use_tie_breaker, index = None):
#   """
#     convert ground truth for use in loss functions
#     Args:
#       y_true: tf.Tensor[] ground truth
...
#   # init a fixed memeory size grid for this prediction scale
#   # [batch, size, size, # of anchors, 1 + 1 + number of anchors per scale]
#   full = tf.zeros([size, size, len_masks, 1 + 4 + 1 + 1], dtype=dtype)
#   # init a grid to use to track which locations have already
#   # been used before (for the tie breaker)
...
#       continue
#     if use_tie_breaker:
#       update_index, update, depth_track, i = _use_tie_breaker(anchors, ious, mask, i, box_id, depth_track, x, y, boxes, classes,
#           update_index, update)
#     else:
...
#         # write the box to the update list
#         update_index, update, i = _update_tensor_arrays(
#             y[box_id], x[box_id], anchor_idx,
#             boxes[box_id], classes[box_id], update_index, update, i)
...
#   boxes = _pad_max_instances(boxes, 50, pad_value = 0, pad_axis = -2)
#   classes = _pad_max_instances(classes, 50, pad_value = -1, pad_axis = -1)
#   confs = _pad_max_instances(confs, 50, pad_value = 0, pad_axis = -1)
#   ious = _pad_max_instances(ious, 50, pad_value = 0, pad_axis = -1)
#   return full, boxes, classes, confs, ious
...
#   boxes_shape = boxes.get_shape()
#   if boxes_shape.ndims == 2:
#     full, boxes, classes, confs, ious = _build_grided_gt(y_true, mask, size, num_classes, dtype, use_tie_breaker)
#   elif boxes_shape.ndims == 3:
#     batches = tf.shape(boxes)[0]
#     full_arr = tf.TensorArray(boxes.dtype, size=0, dynamic_size=True)
#     boxes_arr = tf.TensorArray(boxes.dtype, size=0, dynamic_size=True)
#     classes_arr = tf.TensorArray(boxes.dtype, size=0, dynamic_size=True)
#     confs_arr = tf.TensorArray(boxes.dtype, size=0, dynamic_size=True)
#     ious_arr = tf.TensorArray(boxes.dtype, size=0, dynamic_size=True)
#     for batch in range(batches):
#       full_, boxes_, classes_, confs_, ious_ = _build_grided_gt(y_true, mask, size, num_classes, dtype, use_tie_breaker, index = batch)
#       full_arr = full_arr.write(batch, full_)
#       boxes_arr = boxes_arr.write(batch, boxes_)
#       classes_arr = classes_arr.write(batch, classes_)
#       confs_arr = confs_arr.write(batch, confs_)
#       ious_arr = ious_arr.write(batch, ious_)

#     full = full_arr.stack()
#     boxes = boxes_arr.stack()
#     classes = classes_arr.stack()
#     ious = ious_arr.stack()
#     confs = confs_arr.stack()
#   else:
