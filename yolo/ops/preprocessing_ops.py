...
def fit_preserve_aspect_ratio(image, boxes, width = None, height = None, target_dim = None):
    ...
    image = tf.image.pad_to_bounding_box(image, pad_width//2, pad_height//2, clipper, clipper)
...
def build_grided_gt(y_true, mask, size, num_classes, dtype, use_tie_breaker):
    ...
    classes = tf.one_hot(tf.cast(y_true["classes"], dtype=tf.int32),depth=num_classes,dtype=dtype)
...
def build_batch_grided_gt(y_true, mask, size, num_classes, dtype, use_tie_breaker):
    ...
    full = tf.zeros([batches, size, size, len_masks, num_classes + 4 + 1], dtype=dtype)
