...
sourceFile = open('loss_dump.txt', 'w')
...
def test(data,
         ...
         model=None,
         dataloader=None,
         save_dir=Path('data'),  # for saving images
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        # plt.imshow(img2[0].numpy().astype(np.float32))
        # plt.savefig(f"test{batch_i}.jpg")
        ...
        with torch.no_grad():
            ...
            # print(paths)
            loss_ = compute_loss([x.float() for x in train_out], targets, model)[1]  # box, obj, cls
            loss += loss_
            bo, ob, cl, lo = loss_.cpu().numpy()
            print("%s, box: %0.4f, object: %0.4f, class: %0.4f, total: %0.4f" % (paths[0].replace("/media/vbanna/DATA_SHARE/CV/datasets/COCO_raw/testing_records/images/", "").replace(".jpg", ""), bo, ob, cl, lo), file = sourceFile)
    ...
    print(*(loss.cpu() / len(dataloader)).tolist())
...
if __name__ == '__main__':
    ...
    if opt.task in ['val', 'test']:  # run normally
        test(opt.data,
             ...
             save_dir = "/home/vbanna/Research/ScaledYOLOv4/data"
             )
