...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    # # Trainloader
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, 64, opt, augment=False, pad=0.5, rect=False)

    testloader, _ = create_dataloader(train_path, imgsz, batch_size, 64, opt, augment=False, pad=0.5, rect=False)
    ...
    def dump_loss(a, b):
        k = 0
        for p in a:
            p = (p.sigmoid() - torch.ones_like(p)) ** 2
            k += p.mean()
        return k, k
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                print(imgs.size())
                pred = model(imgs, augment = False)[-1]  # forward

                # loss, loss_items = dump_loss(pred, targets.to(device))  # loss scaled by batch_size
            ...
            scaler.update()
            print(paths, hyp['weight_decay'], list(model.parameters())[-2].grad.square().sum().detach().cpu().numpy())
            print(paths, loss_items.detach().cpu().numpy())
            print("\n\n")
            optimizer.zero_grad()
